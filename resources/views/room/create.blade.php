<x-layout>
    <x-section>
        <div class="w-full text-slate-700">
            <x-title>Add a Room</x-title>
            <form method="POST" action="/rooms" class="bg-white p-4" enctype="multipart/form-data">
                @csrf
                ...
                <div class="flex flex-col lg:w-1/4 mb-2">
                    ...
                    <input type="text" id="title" name="title" class="bg-slate-100 py-1 px-2"
                        placeholder="VIP Room..." value="{{ old('title') }}">
                    @error('title')
                        <p class="text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                <div class="flex flex-col lg:w-1/4 mb-6">
                    ...
                    <input type="text" id="price" name="price" class="bg-slate-100 py-1 px-2"
                        placeholder="8000" value="{{ old('price') }}">
                    @error('price')
                        <p class="text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                <div class="flex flex-col mb-4">
                    ...
                    <input id="inclusions" type="hidden" name="inclusions" value="{{ old('inclusions') }}">
